...
## Post-V2 — Responsive UI and clearer explanations

Completed on 15 September 2026 without reopening the finished V2 engine work. Preserved interrupted UI changes, then validated/committed/pushed separate checkpoints: `4d40e48` for responsive layouts and semantic chart colors (477 tests), `fddc7fa` for bilingual guides, source provenance and metric definitions (491 tests), and `708b7ea` for the final mobile barrier-label and native-control contrast fixes (492-test full run and 33-test focused run). Every checkpoint was verified on the existing remote V2 branch.

Final checks cover all five workspaces on phones, overview widths from 320 to 1920 pixels, both themes/languages, source cards, dropdowns/help, contained book editing and the 3D volatility view. The complete work passes 492 tests, compilation and whitespace checks. Current screenshots, detailed behavior, validation scope and the native dark-table/deployment limitations are recorded in [the UI completion record](ui_responsiveness_2026_09_15.md). No intended UI item remains unfinished; deployment is separate from the requested branch push.

## Post-V2 — Introductory menu

Completed on 17 September 2026. Preserved the interrupted welcome-page implementation after verifying local/remote `3622c28`. Added Start / Accueil as the default entry point, a guided first visit, direct cards to all five workspaces, a current-portfolio summary and four bilingual FAQs. Navigation preserves the book; provider requests wait until entry to an analytical workspace. Finished responsive card alignment and accessible heading order.

503 tests passed, including eleven welcome/navigation regressions and an actual Streamlit rendering check replacing an obsolete source-text width restriction. Checkpoint `9dbcc21` was committed/pushed and its remote SHA verified. Final documentation, screenshots and viewport checks are recorded in [the welcome-page completion record](welcome_2026_09_17.md).
