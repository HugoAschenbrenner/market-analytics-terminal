...
FIELDS = dict(id="", ticker="", asset_class="Equity", currency="USD", quantity=1., price=100.,
              ...
              volatility=.20, coupon=.04, yield_rate=.04, option_type="Call", weight=np.nan)
...
def demo_book(name="balanced"):
    ...
    equity = position("spy", "SPY", "Equity", 500, 550, sleeve="Equity")
    ...
    return PositionBook(validate_book(pd.DataFrame(rows)), name=name)
