"""V2-only presentation. The frozen V1 theme remains untouched."""
import streamlit as st

TOKENS = {
    "dark": dict(background="#0b0d10", surface="#12161b", surface_secondary="#1b2128", border="#39414b", text_primary="#e9edf2", text_secondary="#b0b8c3", positive="#38c6a3", negative="#ff7b89", warning="#eabc63", accent="#e8b66f", grid="#252c34", chart_background="#12161b", violet="#b5a0ff"),
    "light": dict(background="#f1f4f8", surface="#ffffff", surface_secondary="#edf0f4", border="#bbc9d9", text_primary="#16293f", text_secondary="#506580", positive="#067359", negative="#be3046", warning="#925c00", accent="#855000", grid="#e0e7ef", chart_background="#ffffff", violet="#7252bd"),
}
...
def apply_theme(theme):
    ...
    .block-container {{padding:.6rem clamp(.7rem,1.6vw,2rem) 2rem;max-width:2200px}}
    ...
    [data-testid="stMetric"] {{height:100%;background:var(--surface);border:1px solid var(--border);padding:.5rem .7rem}}
    [data-testid="stMetricValue"] {{font-size:clamp(1.2rem,1.5vw,1.6rem);line-height:1.25}}
    ...
    [data-testid="stMetricLabel"] {{height:auto;min-height:1.5rem;overflow:visible}}
    ...
    .st-key-welcome-hero {{margin-top:.7rem;padding:clamp(.8rem,1.5vw,1.5rem);border:1px solid var(--border);border-radius:3px;background:linear-gradient(135deg,var(--surface),var(--surface-secondary))}}
    .welcome-kicker {{font-size:.72rem;font-weight:700;letter-spacing:.13em;color:var(--accent);margin-bottom:.75rem}}
    .welcome-title {{font-size:clamp(1.6rem,2.8vw,2.5rem)!important;line-height:1.08;letter-spacing:-.035em;max-width:22ch;padding:0 0 1rem!important}}
    ...
    [class*="st-key-welcome-card-"] {{height:100%;padding:1.15rem;border:1px solid var(--border);border-radius:3px;background:var(--surface)}}
    ...
    .session-item {{background:var(--surface);border:1px solid var(--border);padding:.45rem .6rem;font-size:.75rem}}
    .session-item b {{display:block;font-size:.8rem}} .session-item small {{color:var(--text-secondary)}}
    .stApp {{font-variant-numeric:tabular-nums}}
    @media(max-width:700px) {{
      .st-key-terminal-header [data-testid="stHorizontalBlock"] {{grid-template-columns:minmax(0,1fr) 75px 95px}}
      .st-key-terminal-header [data-testid="stColumn"]:nth-child(2) {{grid-row:2;grid-column:1/-1!important}}
      .st-key-terminal-header [data-testid="stColumn"]:first-child {{grid-column:auto!important}}
