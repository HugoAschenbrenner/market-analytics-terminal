...
from app_pages import overview, markets, risk_lab, derivatives_lab, financing, welcome, equity_derivatives, structured_lab

def render():
    from services.market_data import ensure_market
    state=get_state()
    if page_from_query() not in ('welcome','equity-derivatives'):
        ensure_market(state)
    global_header(state)
    from components.version_switch import render_version_switch
    render_version_switch("v2", state.ui.language, state.ui.theme)
    if state.ui.page not in ('welcome','equity-derivatives'):
        book_selector(state)
        from components.education import render_workspace_guide
        render_workspace_guide(state)
    PAGES={'welcome':welcome,'overview':overview,'markets':markets,'risk':risk_lab,'derivatives':derivatives_lab,'financing':financing,'equity-derivatives':equity_derivatives,'structured-products':structured_lab}
    try:
