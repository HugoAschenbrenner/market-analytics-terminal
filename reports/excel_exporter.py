...
def generate_fixed_income_risk_report(
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        summary_rows = [
            ...
            ("Total Clean Market Value in Base Currency", summary.get("total_clean_market_value")),
            ("Total Full Market Value in Base Currency", summary.get("total_full_market_value")),
            ("Accrued Interest Amount in Base Currency", summary.get("total_accrued_interest_amount")),
        ...
        methodology_items = [
            ...
            "Local clean market value equals clean price / 100 multiplied by notional in each bond currency.",
            "Local full market value equals dirty price / 100 multiplied by notional and includes accrued interest.",
            "DV01 and duration/convexity scenario P&L use full market value.",
...
def generate_portfolio_risk_report(
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        _write_key_value_sheet(
            ...
            data=summary,
            commentary=[
                "Portfolio risk metrics are calculated from historical simple returns under explicit frequency and horizon assumptions.",
                "VaR and CVaR are historical, use the horizon recorded in the summary, and are reported as positive loss numbers.",
        ...
        methodology_items = [
            ...
            "Arithmetic annualized return equals average periodic return multiplied by the explicitly selected periods-per-year factor.",
            "Geometric CAGR is calculated from compounded portfolio wealth over the sample length.",
            "Annualized volatility equals periodic volatility multiplied by the square root of the explicitly selected periods-per-year factor.",
